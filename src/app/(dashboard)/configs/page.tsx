...
const sourceStyles: Record<string, string> = {
  StatusReport: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800",
  ConfigFile: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800",
  ManualUpload: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800",
};
                ...
                {configs.map((config) => (
                  <tr key={config.id} className="group transition-colors hover:bg-[var(--muted)]/50">
                    <td className="px-6 py-4">
                      <Link href={`/configs/${config.id}`} className="text-sm font-semibold text-blue-600 group-hover:text-blue-800 dark:text-blue-400">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${sourceStyles[config.triggerType || ""] || "bg-[var(--muted)] text-[var(--muted-foreground)]"}`}>
                        {config.triggerType || "-"}
                      </span>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-[var(--foreground)]">
                      {new Date(config.capturedAt).toLocaleDateString("ko-KR")}
                    </td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-[var(--muted-foreground)]">
                      {new Date(config.uploadedAt).toLocaleDateString("ko-KR")}
                    </td>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <Link
                ...
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${
                  p === page ? "bg-blue-600 text-white" : "text-[var(--foreground)] hover:bg-[var(--accent)]"
                }`}
              >
                {p}
              </Link>
